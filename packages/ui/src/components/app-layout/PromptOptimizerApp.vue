    ...
    >
        ...
        <template v-else>
            <MainLayoutUI>
                ...
                <template #core-nav>
                    <AppCoreNav :workspace-path="activeWorkspaceContextPath" />
                </template>
...
import { type IPromptService, type PromptAssetBinding, type PromptSessionOrigin, type PromptRecordChain, type PatchOperation, type Template, type TemplateType, type FunctionMode, type BasicSubMode, type ImageSubMode, type OptimizationMode, type ToolDefinition, type ContextMode } from "@prompt-optimizer/core";
...
// 纯解析函数：从工作区路径提取模式和子模式；非工作区路径沿用最近工作区上下文
...
// 上下文模式（上下文/Pro 模式已删除，恒为 system）
const contextMode = ref<ContextMode>("system");

// 变量管理器实例（自定义全局变量，供提示词渲染/预览使用）
const variableManager = useVariableManager(services);

// 临时变量管理器：
// - Image：按子模式 session store 持久化（刷新不丢；子模式之间隔离）
// - Basic：维持旧行为，仅内存态
const tempVarsManager = useTemporaryVariables();

// 使用聚合变量管理器（供提示词预览面板渲染变量）
...
watch(
    ...
    (newTemplate) => {
        ...
        // ⚠️ 仅 Basic 模式使用此迭代模板；Image 使用独立的模板管理
        if (routeFunctionMode.value === 'image') return;
...
watch(
    ...
    (newContent) => {
        ...
        // 🔧 仅 Basic 模式使用此 testContent；Image 模式没有 testContent
        if (routeFunctionMode.value === 'image') return;
...
// 提供 openPromptPreview 接口（供 Image 工作区打开提示词预览面板）
...
// 8. 监听高级模式和优化模式变化
// 上下文模式（Pro）已删除：不再需要为高级模式写入默认上下文
...
// 模型管理界面已删除（模型配置直接读取 ~/.config/ai.yml）
// 模型选择由各工作区通过 TextModelQuickSwitch 完成，无需配置入口
...
async function navigateToSubModeKey(
  ...
  // SubModeKey 格式：'basic-system' | 'image-text2image'
  const [mode, subMode] = toKey.split('-') as [
    FunctionMode,
    BasicSubMode | ImageSubMode
  ]
