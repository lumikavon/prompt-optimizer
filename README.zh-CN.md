...
<div align="center">
  ...
  <img src="images/demo/pro-variable-bargaining-reply.png" alt="闲鱼砍价回复模板演示" width="85%">
  <br>
...
- 📝 **基础模式**：双优化目标——系统提示词优化与用户提示词优化，覆盖角色扮演、语气、工作流与通用任务提示词
- 🖼️ **图像模式**：文生图（T2I）、图生图（I2I）与多图工作区，支持变量感知输入、参考图与提示词优化
- 🔄 **分析与评估**：支持分析、评估、带反馈评估与基于评估结果的改写，帮助判断提示词是否真的改进
- 🤖 **多模型集成**：支持OpenAI、Gemini、DeepSeek、Grok、智谱AI、SiliconFlow、MiniMax等主流AI模型
- 🌱 **提示词来源**：可从手写、模板、本地导入或提示词库导入码开始
- 🛠️ **工具调用支持**：可配置支持 Function Calling 的模型（OpenAI、Gemini 等）并为提示词流程定义工具定义（Schema）
...
### 图像模式
- 🖼️ **文生图（T2I）**：编写能引导文生图模型的提示词
- 🎨 **图生图（I2I）**：基于本地参考图片编写提示词，提供风格与内容引导
- 🖼️ **多图**：使用多张输入图共同约束主体关系、顺序语义与最终生成目标
- 🔌 **多模型支持**：集成Gemini、Seedream、Grok等主流AI模型用于提示词优化
- 📊 **变量感知输入**：在提示词中定义变量，优化前可预览变量如何解析
- 📝 **模板与模型**：复用提示词模板，并在每个工作区选择优化模型
- 🔍 **分析与评估链路**：支持分析、评估、带反馈评估和基于评估结果的智能改写
- 🛠️ **工具调用支持**：Function Calling集成，支持OpenAI和Gemini工具调用

### 提示词来源与模板
- 🌱 **可选提示词来源**：从手写、模板、本地文件或 [Prompt Garden 提示词库](https://garden.always200.com) 带入提示词
- 📥 **导入与收集**：在有来源信息时，连同元数据、媒体和示例一起导入
- 📝 **模板管理**：管理可复用的提示词模板，支持变量占位符用于批量个性化
...
- [x] 桌面应用发布
- [x] MCP服务发布
- [x] 图像模式：文生图（T2I）、图生图（I2I）和多图工作区支持
- [x] 图像模式变量感知输入
- [x] 提示词模板与 Function Calling 配置
- [ ] 支持工作区/项目管理
